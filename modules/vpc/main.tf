...
locals {
  ...
  vpc_route_table_subnet_assocs = flatten([
    for rt in var.route-tables: [
      for subnet in rt.subnet: {
        subnet = subnet
        route_table = rt.name
      }
    ]
    if lookup(rt, "subnet", null) != null
  ])
...
resource "aws_route_table_association" "sub_assoc" {
  for_each = { 
    for assoc in local.vpc_route_table_subnet_assocs: assoc.subnet => assoc
  }

  subnet_id = local.subnet_ids["${var.prefix-name-tag}${var.vpc.name}-${each.value.subnet}"]
  route_table_id = local.vpc_route_table_ids["${var.prefix-name-tag}${var.vpc.name}-${each.value.route_table}"]
}
...
resource "aws_security_group" "this" {
  for_each = { for sg in var.security-groups: sg.name => sg }
...
resource "aws_eip" "ec2_eip" {
  ...
  vpc = true
  tags = merge({ Name = "${var.prefix-name-tag}-eip" }, var.global_tags)
}
